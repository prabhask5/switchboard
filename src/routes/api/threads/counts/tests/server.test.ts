...
describe('POST /api/threads/counts', () => {
	...
	it('handles mix of rules and no-rules panels', async () => {
		...
		const panels = [
			...
			{
				name: 'Social',
				rules: [{ field: 'from', addresses: ['@social.com'], action: 'accept' }]
			},
			{ name: 'Inbox B', rules: [] }
		];
	...
	it('makes separate API calls per rules panel', async () => {
		...
		const panels = [
			...
			{ name: 'C', rules: [{ field: 'from', addresses: ['@c.com'], action: 'accept' }] }
		];
		...
		const panels = [
			{
				name: 'Mixed',
				rules: [
					{ field: 'from', addresses: ['@a.com'], action: 'accept' },
					{ field: 'from', addresses: ['@b.com'], action: 'accept' },
